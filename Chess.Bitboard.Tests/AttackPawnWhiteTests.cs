...
namespace Chess.Lib.Tests
{
	...
	public class AttackPawnWhiteTests
	{
		...
		[TestMethod]
		public unsafe void TestWhiteEnPassant()
		{
			BoardStruct* b = (BoardStruct*)Board.Board_Create();
			Board.Board_SetPiece((IntPtr)b, 37, Board.PIECE_PAWN, Board.COLOR_WHITE);
			Board.Board_SetPiece((IntPtr)b, 36, Board.PIECE_PAWN, Board.COLOR_BLACK);
			b->EnPassantTile = 44;

			ulong attacks = Moves.Moves_GetAttacks((IntPtr)b, 37);
			Assert.AreEqual((ulong)0x500000000000, attacks);

			ulong moves = Moves.Moves_GetMoves((IntPtr)b, 37);
